...
void Platform::loadAnimations()
{
	...
	this->animMap[std::pair("idle", true)].frames = loadAnimation(3, 3, 0, 0, 0);
	this->animMap[std::pair("idle", true)].animDelay = 0.3f;
	this->animMap[std::pair("idle", true)].pauseDelay = 0.f;
	this->loadBBoxes();
}

std::vector<sf::IntRect> Platform::loadAnimation(int numFrames, int  pitch, int startCol, int startRow, int pitchColBegin)
{
