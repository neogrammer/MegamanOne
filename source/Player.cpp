...
void Player::undoMapMovement(sf::View& gview_)
{
	if (mapMovedLeft)
	{
		gview_.move({ -250.f * gTime,0.f });
	}
	else if (mapMovedRight)
	{
		gview_.move({ 250.f * gTime,0.f });
	}
}

void Player::resolveBothAxes(const rect<float>& r, ResolutionDir resDir, ResolutionDir resDir2)
{
	line<float> c2c = { {bbRect().middle()},{r.middle()} };
	auto playerOutPt = intersects(c2c, bbRect());
	if (playerOutPt.size() == 0) return;
	auto tileInPt = intersects(c2c, r);
	if (tileInPt.size() == 0) return;
	line<float> gapLine = { olc::vf2d{playerOutPt[0].x, playerOutPt[0].y}, olc::vf2d{tileInPt[0].x, tileInPt[0].y } };
	auto nextPlayerOutPt = intersects(c2c, getNextRect());
	if (nextPlayerOutPt.size() == 0) return;

	line<float> collideLine = { olc::vf2d{playerOutPt[0].x, playerOutPt[0].y }, olc::vf2d{nextPlayerOutPt[0].x, nextPlayerOutPt[0].y } };
	float ratioAdjustment = (gapLine.length()) / collideLine.length();

	if (std::isnan(ratioAdjustment)) return;

	if (ratioAdjustment < 0.f) { ratioAdjustment = 0.f; }
	if (ratioAdjustment > 1.f) { ratioAdjustment = 1.f; }
	this->setVelocity({ this->getVelocity().x * (ratioAdjustment), this->getVelocity().y * (ratioAdjustment) });
}

void Player::resolveOneAxis(const rect<float>& r, ResolutionDir resDir)
{
	line<float> c2c = { {bbRect().middle()},{r.middle()} };
	auto playerOutPt = intersects(c2c, bbRect());
	if (playerOutPt.size() == 0) return;
	...
	if (ratioAdjustment < 0.f) { ratioAdjustment = 0.f; }
	if (ratioAdjustment > 1.f) { ratioAdjustment = 1.f; }
	if (resDir == ResolutionDir::Down || resDir == ResolutionDir::Up)
	{
		this->setVelocity({ this->getVelocity().x, this->getVelocity().y * (ratioAdjustment) });
	}
	else
	{
		this->setVelocity({ this->getVelocity().x * (ratioAdjustment), this->getVelocity().y});
	}
}

void Player::resolve(const rect<float>& r, olc::vi2d& norm)
{
	line<float> c2c = { {bbRect().middle()},{r.middle()}};
	auto playerOutPt = intersects(c2c, bbRect());
	if (playerOutPt.size() == 0) return;
...
void Player::input(sf::View& gview_, std::vector<Tile>& tiles)
{
	mapMoved = false;
	mapMovedLeft = false;
	mapMovedRight = false;
...
void Player::handleMapCollisions(std::vector<Tile>& tiles)
{
	...
	bool noCanJump = false;

	if ((right_down || left_down) && !isAffectedByGravity() && getVelocity().y != 0.f)
	{
		if (checkBelow(tiles))
		{
	...
	auto bRect = this->getBroadRect();
	auto nextRectPersistent = this->getNextRect();
	bool collidedBelow = false;
	for (auto& t : tiles)
	{
		if (overlaps(bRect, t.bbRect()))
		{
			ResolutionDir resDir = ResolutionDir::None;
			ResolutionDir resDir2 = ResolutionDir::None;
			if (overlaps(this->getNextRect(), t.bbRect()))
			{
				olc::vi2d contactNormal = { 0,0 };


				resDir = ResolutionDir::None;
				resDir2 = ResolutionDir::None;
				...
				line<float> contactNormalLine = { {getBBCenter()}, {getBBCenter() + getVelocity() * getBBSize()} };

				if (overlaps(contactNormalLine, t.bbRect()))
				{
					auto pts = intersects(contactNormalLine, t.bbRect());
					// invert vector on axis colliding into
					ray<float> myRay = { {pts[0]}, {pts[0] - getBBCenter()} };
					if (myRay.direction.x == 0.f)
					{

					}
					else
					{
						contactNormal.x = (int)(myRay.direction.x / myRay.direction.x);
						if (myRay.direction.x > 0.f)
						{
							contactNormal.x = -contactNormal.x;
						}
					}
					if (myRay.direction.y == 0.f)
					{

					}
					else
					{
						contactNormal.y = (int)(myRay.direction.y / myRay.direction.y);
						if (myRay.direction.y > 0.f)
						{
					...
					std::cout << "normalx: " << contactNormal.x << ", y: " << contactNormal.y << std::endl;
					...
					// got normal in contactNormal now
					if (contactNormal.y == -1 && contactNormal.x == 0)
					{
						this->setVelocity({ this->getVelocity().x, 0.f });

						this->setPos({ this->getPos().x, t.getPos().y - this->getBBSize().y - 0.0001f });
					}
					else if (contactNormal.y == 1 && contactNormal.x == 0)
					{
						this->setVelocity({ this->getVelocity().x, 0.f });

						this->setPos({ this->getPos().x, t.getPos().y + 0.0001f });
					}
					else if (contactNormal.x == -1 && contactNormal.y == 0)
					{
						this->setVelocity({ 0.f, this->getVelocity().y });

						this->setPos({ t.getPos().x - this->getBBSize().x - 0.0001f, this->getPos().y });
					}
					else if (contactNormal.x == 1 && contactNormal.y == 0)
					{
						this->setVelocity({ 0.f,  this->getVelocity().y });

						this->setPos({ t.getPos().x + 0.0001f , this->getPos().y });
					}



					else if (contactNormal.y == -1 && contactNormal.x == 1)
					{
						this->setVelocity({ this->getVelocity().x, 0.f });

						this->setPos({ this->getPos().x, t.getPos().y - this->getBBSize().y - 0.0001f });

						if (overlaps(getNextRect(), t.bbRect()))
						{
					...
					else if (contactNormal.x == -1 && contactNormal.y == 1)
					{
						this->setVelocity({ this->getVelocity().x, 0.f });
...
bool Player::checkBelow(std::vector<Tile>& tiles)
{
	rect<float> tmp{ bbRect() };
	tmp.pos.y += 10.f;
	bool collided = false;
